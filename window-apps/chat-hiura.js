...
export function firstOpen() {
  ...
  !(function (t, e) {
    return "object" == typeof exports && "undefined" != typeof module
      ? e(exports)
    ...
    function le(t, e, r, i, n, a) {
      ...
      r
        ...
        : ((w = (s = Vd(t)).x + (~b[0].indexOf("%") ? (w / 100) * s.width : w)),
          (x =
            s.y + (~(b[1] || b[0]).indexOf("%") ? (x / 100) * s.height : x))),
        i || (!1 !== i && h.smooth)
          ? ((y = w - f),
            (T = x - d),
  ...
  const socket = new WebSocket(
    /*'wss://gddr51pi43.execute-api.ap-southeast-1.amazonaws.com/dev'*/ "wss://websocket.hiura.site"
  );
  const murl = "https://chat.hiura.site";
  let clientId = null;
  let clientName = null;
  ...
  function checkRateLimit() {
    if (messageCount >= messageLimit) {
      ...
      if (!rateLimitTimer) {
        rateLimitTimer = setTimeout(resetRateLimit, timeWindow);
      }
      return false;
    }
    return true;
  ...
  socket.addEventListener("message", function (event) {
    ...
    const messageType = sender === clientId ? "outgoing" : "incoming";

    if (sender === clientId) {
      removeLoadingBubble();
    }

    displayMessage(message, messageType, senderName, profilePicture);
  });

  function sendMessage() {
    const message = messageInput.value.replace(/\n/g, "⇩");
    if (message.trim() !== "" && checkRateLimit()) {
      socket.send(
        JSON.stringify({
          action: "sendMessage",
          data: {
  ...
  function createMessageElement(
    ...
    profileIcon.src = `${murl}/${profilePicture}`;
    profileIcon.onerror = () => {
      profileIcon.src = "/css/icons/globe.png";
    };
    profileIcon.alt = `${senderName}'s profile picture`;
    ...
    messageContainer.style.alignItems =
      type === "outgoing" ? "flex-end" : "flex-start";
    ...
    nameElement.style.textAlign = type === "outgoing" ? "right" : "left";

    // Message bubble
    const bubbleElement = document.createElement("div");
    bubbleElement.className = "message-bubble lia-card";
    bubbleElement.textContent = message;
    bubbleElement.innerHTML = bubbleElement.innerHTML.replaceAll("⇩", "<br>");
    bubbleElement.style.backgroundColor =
      type === "outgoing" ? "#d4ba8b70" : "#ebebeb70";
    ...
    if (type === "outgoing") {
      ...
      bubbleElement.style.marginRight = "0";
      // profileIcon.style.margin = '0.5em 0 0 -0.75em';
    } else {
      messageElement.style.marginLeft = "0";
      messageElement.style.justifyContent = "flex-start";
      bubbleElement.style.marginLeft = "0";
      // profileIcon.style.margin = '0.5em -0.75em 0 0';
    }

    // if (isLoading) {
    //   bubbleElement.id = `loading-${pendingMessageId}`;
    //   bubbleElement.style.opacity = '0.6';
    // }

    // Append name and message bubble to the container
    messageContainer.appendChild(nameElement);
    if (!isLoading) {
      messageContainer.appendChild(bubbleElement);
    }

    // Append profile icon and message container based on type
    if (type === "outgoing") {
  ...
  function displayMessage(message, type, senderName, profilePicture) {
  ...
  function showLoadingBubble(message) {
    pendingMessageId += 1;
    activeLoadingBubbleId = `loading-${pendingMessageId}`;
    ...
    loadingElement.id = activeLoadingBubbleId;
  ...
  function removeLoadingBubble() {
    if (activeLoadingBubbleId) {
      const loadingElement = document.getElementById(activeLoadingBubbleId);
      if (loadingElement && chatWindow.contains(loadingElement)) {
  ...
  //   document.querySelector("span.hiura").innerHTML =
